...
def get_api_hot_items():
    ...
    try:
        ...
        filtered_data = [
            {"id": item_id, "high": details.get("avgHighPrice"), "low": details.get("avgLowPrice"),
             "high_vol": details.get("highPriceVolume"), "low_vol": details.get("lowPriceVolume")}
...
def find_averages(data):
    ...
    averaged_data = [
        {
            "id": item["id"],
            "avg_price": round(((item.get("high", 0) or 0) + (item.get("low", 0) or 0)) / 2, 2),  # Average of high and low price
            "avg_vol": round(((item.get("high_vol", 0) or 0) + (item.get("low_vol", 0) or 0)) / 2, 2)  # Average of high and low volume
        }
        for item in data
    ]
