...
export default function ThemeSwitch() {
  ...
  // Use an effect to persist the theme when the component mounts
  useEffect(() => {
    // Check if the user prefers dark mode or if there's an existing theme in localStorage
    const prefersDarkMode = window.matchMedia('(prefers-color-scheme: dark)').matches
    const savedTheme = localStorage.getItem('theme')

    // If there's a saved theme preference, use that, otherwise use the system preference
    setEnabled(savedTheme ? savedTheme === 'dark' : prefersDarkMode)
  ...
  // Update the theme whenever the switch is toggled
  const toggleTheme = () => {
    setEnabled(!enabled)
  ...
  return (
    ...
    >
      ...
      <span className="pointer-events-none relative inline-block size-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out group-data-[checked]:translate-x-5">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6 text-indigo-600">
